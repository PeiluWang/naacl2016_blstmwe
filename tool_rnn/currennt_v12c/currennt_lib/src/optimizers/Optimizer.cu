...
namespace optimizers {
    ...
    {
		// store best we weights
		layers::InputLayer<TDevice> *inputlayer = dynamic_cast<layers::InputLayer<TDevice>*>(m_neuralNetwork.layers()[0].get());
		thrust::copy(inputlayer->weWeights().begin(),inputlayer->weWeights().end(), v_best_weweights.begin());
		thrust::copy(inputlayer->featWeights().begin(),inputlayer->featWeights().end(), v_best_featweights.begin());
        for (size_t i = 1; i < m_neuralNetwork.layers().size() - 1; ++i) {
            layers::TrainableLayer<TDevice> *layer = dynamic_cast<layers::TrainableLayer<TDevice>*>(m_neuralNetwork.layers()[i].get());
            if (layer) 
    ...
    {
        // restore best we weights
		layers::InputLayer<TDevice> *inputlayer = dynamic_cast<layers::InputLayer<TDevice>*>(m_neuralNetwork.layers()[0].get());
		thrust::copy(v_best_weweights.begin(),v_best_weweights.end(), inputlayer->weWeights().begin());
		thrust::copy(v_best_featweights.begin(),v_best_featweights.end(), inputlayer->featWeights().begin());
		for (size_t i = 1; i < m_neuralNetwork.layers().size() - 1; ++i) {
        	layers::TrainableLayer<TDevice> *layer = dynamic_cast<layers::TrainableLayer<TDevice>*>(m_neuralNetwork.layers()[i].get());
            if (layer)
        ...
        , m_curTestClassError         (0)
    {
		...
		layers::InputLayer<TDevice> *inputlayer = dynamic_cast<layers::InputLayer<TDevice>*>(m_neuralNetwork.layers()[0].get());
		v_best_weweights=inputlayer->weWeights();
		v_best_featweights=inputlayer->featWeights();
        for (size_t i = 1; i < m_neuralNetwork.layers().size()-1; ++i) {
        	layers::TrainableLayer<TDevice> *layer = dynamic_cast<layers::TrainableLayer<TDevice>*>(m_neuralNetwork.layers()[i].get());
            if (layer)
    ...
    bool Optimizer<TDevice>::train()
    {
        if (!m_finished) {
            ++m_curEpoch;
			bool isclassification=m_trainingSet.isClassificationData();
            ...
            if (!m_validationSet.empty() && m_curEpoch % m_validateEvery == 0) {
				...
				if(isclassification){
					m_curValidationError=m_curValidationClassError;
				}
