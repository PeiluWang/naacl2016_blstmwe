...
namespace data_sets {
        ...
        , m_curFirstSeqIdx   (-1)
    {
		...
		bool hasmultidata=Configuration::instance().hasMultiData();
		int suffixid=-1;
		std::cout<<std::endl;
		int seqid=0;
		m_totalSequences=0;
		...
		while(true){
			if(suffixid<0){// open *.nc file
				...
				char buffer[100];
				sprintf(buffer,"%s.%d",ncfile.c_str(),suffixid);
				if ((ret = nc_open(buffer, NC_NOWRITE, &ncid)))
					break;
				std::cout<<"load file: "<<ncfile<<std::endl;
			}
			// extract the patterns from the *.nc file
			try {
					m_isClassificationData = true;
					...
					int part_totalSequences = internal::readNcDimension(ncid, "seq_num");
					m_totalSequences+=part_totalSequences;
					...
					int targetsBegin = 0;
					for (int i = 0; i < part_totalSequences; ++i) {
