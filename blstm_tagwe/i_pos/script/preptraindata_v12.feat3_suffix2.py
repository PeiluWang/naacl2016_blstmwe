...
#inputfeats包含：大小写，是否包含hyphen，是否包含数字，suffix2的one-hot representation
import os
import codecs
import sys
import traceback
...
def main():
		...
		for f in fs:
			...
			for line in fi:
				...
				for tok in toks:
					...
					tagid=tagdict[ts[1]]
					seninfo.append((wordid,tagid,lower_flag,has_hyphen,has_num,suffix2id))
	...
	for senwords in sentences:
		...
		for i in range(sample_num):
			...
			has_num=senwords[i][4]
			suffix2id=senwords[i][5]
			...
			ncvar_inputfeats[frame_index,2]=has_num
			ncvar_inputfeats[frame_index,3+suffix2id]=1
